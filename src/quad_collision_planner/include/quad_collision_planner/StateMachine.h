...
struct Run: fsm::state<Run,QuadcopterPlanner>{
  ...
  fsm::result react( const EvClock &evClock ){
    ...
    else if(context<QuadcopterPlanner>().collisionData_.collision){
      return transit<Avoid>();
    }
    else{
...
// Running state. Send the targets one by one 
struct Avoid: fsm::state<Avoid,QuadcopterPlanner>{
  
  Avoid(my_context ctx):my_base(ctx){
#if VERBOSE_MODE
    std::cout << "Entered the Avoid State" << std::endl;
#endif
  }
  ...
  fsm::result react( const EvClock &evClock ){
    if(context<QuadcopterPlanner>().reached2_){
      ...
      tf::Vector3 steerVector = context<QuadcopterPlanner>().collisionData_.steerVector;
      int avoidStep = context<QuadcopterPlanner>().collisionData_.avoidStep;
      
      pPose->position.x=gps.x+avoidStep*step.x*steerVector.x();
      pPose->position.y=gps.y+avoidStep*step.y*steerVector.y();
      pPose->position.z=gps.z+avoidStep*step.z*steerVector.z();
      pPose->orientation=target.orientation;
...
struct Wait: fsm::state<Wait,QuadcopterPlanner>{
  ...
  fsm::result react( const EvClock &evClock ){
	...
	context<QuadcopterPlanner>().reached2_ = true;
      ...
      if(!context<QuadcopterPlanner>().collisionData_.collision){
	return transit<Run>();
      }else{
	return transit<Avoid>();
      }
